...
async def start_match():
    ...
    print('Depose marron        ')
    print('*********************')
    await propulsion.pointTo(poses.pose_construction, 2.0)
    await propulsion.moveToRetry(poses.pose_construction, 1.0)
    await propulsion.pointTo(poses.depose_marron, 2.0)
    ...
    await propulsion.pointTo(poses.tir_cerises, 2.0)
    ...
    if assiette_1 == True and five_secs_limit == False:
        ...
        await asyncio.sleep(0.2)
        await propulsion.translation(-0.25, 1.0)
...
async def goto_final_pose():
    ...
    print('******************')
    print('******************')
    await propulsion.pointTo((1.0, 0.3), 5.0)
    await propulsion.moveToRetry((1.0, 0.3), 1.0)
    #await propulsion.faceDirection(180, 5.0)
    await propulsion.pointTo(poses.zone_fin, 5.0)
    await propulsion.moveToRetry(poses.zone_fin, 0.8)
    in_zone = True
    await propulsion.faceDirection(0, 5.0)
