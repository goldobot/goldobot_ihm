...
pose_asc_av_stage3_predepose = 0xF000
...
@robot.sequence
async def prise_av(do_prep=True):
    short_timeout = 0.1

    if do_prep:
        prep_task = asyncio.create_task(prise_av_prep())
        try:
...
async def depose_3_etages_av_avec_predepose():
    ...
    await asyncio.sleep(short_timeout)
    # FIXME : DEBUG : EXPERIMENTAL!
    asc_av_task1 = asyncio.create_task(ascenseur_av_stage3_predepose())
    ...
    try:
        await propulsion.reposition(0.2, 0.2)
        #await propulsion.reposition(0.19, 0.2)
    except:
        await propulsion.reposition(0.2, 0.2)
        #await propulsion.reposition(0.19, 0.2)
    ...
    # FIXME : DEBUG : EXPERIMENTAL!
    #asc_av_task2 = asyncio.create_task(ascenseur_av_stage3_depose())
    #await ascenseur_av_stage3_depose()
    #await asyncio.sleep(short_timeout)
