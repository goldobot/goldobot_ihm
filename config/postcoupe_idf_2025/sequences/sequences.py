...
async def experimental_niv3_la_totale():
    ...
    print ("******************************************************")
    print ("* Construction3 niv2")
    print ("******************************************************")
    # construction3 niv2
    await experimental_niv2_final()
    await asyncio.sleep(long_timeout)
    ...
    if misc.match_time()>90.0:
        await propulsion.pointTo(poses.Act4_final, turn_speed)
        ...
        if (propulsion.pose.position.x<0.6):
...
async def experimental_prise1_double():
    ...
    prep_task_av = asyncio.create_task(actuators_front.prise_av_prep())
    try:
    ...
    await asyncio.sleep(short_timeout)
    await actuators_front.prise_av(False)
    await asyncio.sleep(short_timeout)
    await prep_task_av
    # FIXME : DEBUG : EXPERIMENTAL!
    asc_av_task = asyncio.create_task(actuators_front.ascenseur_av_transport())
...
async def experimental_prise2():
    ...
    ## prise3 AV
    prep_task = asyncio.create_task(actuators_front.prise_av_prep())
    propulsion.adversary_detection_enable = True
    ...
    await asyncio.sleep(short_timeout)
    #await actuators_front.bras_av_prise()
    #await asyncio.sleep(long_timeout)
    await prep_task
    # FIXME : DEBUG : EXPERIMENTAL!
    await asc_av_task
...
async def experimental_prise3():
    ...
    if poses == pos.YellowPoses:
        prise_4 = (0.525, -0.675, 0)
    elif poses == pos.BluePoses:
        prise_4 = (0.525,  0.675, 0)
    else:
        raise RuntimeError('Side not set')
    face_direction_4 = 180

    ## prise4 AV
    await actuators_front.ventouses_av_ext_attrape()
    await actuators_front.ventouses_av_int_attrape()
    await asyncio.sleep(short_timeout)
    prep_task = asyncio.create_task(actuators_front.prise_av_prep())
    propulsion.adversary_detection_enable = True
    #await propulsion.pointTo(prise_4, turn_speed)
    #await asyncio.sleep(short_timeout)
    #await propulsion.moveToRetry(prise_4, long_speed)
    #await asyncio.sleep(short_timeout)
    #await propulsion.faceDirection(face_direction_4, turn_speed)
    #await asyncio.sleep(short_timeout)
    await propulsion.faceDirection(face_direction_4, turn_speed)
    await asyncio.sleep(short_timeout)
    p0_x = propulsion.pose.position.x
    ...
    try:
        await propulsion.trajectorySpline(prise4_traj, speed=long_speed)
    except:
        await oldseq.trajectory_spline_exception()
        await propulsion.pointTo(prise_4, turn_speed)
        await propulsion.moveToRetry(prise_4, long_speed)
...
@robot.sequence
async def experimental_goto_construction3_niv2():
    global poses
    ...
    if poses == pos.YellowPoses:
        #depose_3 = (1.680, -0.725, 0)
        depose_3 = (1.800, -0.725, 0)
    elif poses == pos.BluePoses:
        #depose_3 = (1.680,  0.725, 0)
        depose_3 = (1.800,  0.725, 0)
    else:
        raise RuntimeError('Side not set')
    #await propulsion.pointTo(depose_3, turn_speed)
    #await asyncio.sleep(short_timeout)
    #await propulsion.moveToRetry(depose_3, long_speed)
    #await asyncio.sleep(short_timeout)
    #await propulsion.faceDirection(0, turn_speed)
    #await asyncio.sleep(short_timeout)
    await propulsion.faceDirection(0, turn_speed)
    ...
    try:
        await propulsion.trajectorySpline(depose3_traj, speed=long_speed)
    except:
        await oldseq.trajectory_spline_exception()
        await propulsion.pointTo(prise_4, turn_speed)
        await propulsion.moveToRetry(prise_4, long_speed)
...
@robot.sequence
async def experimental_construction_niv2_au_sol_av():
    short_timeout = 0.1
    long_timeout = 0.5
    ...
    await actuators_front.bras_av_up()
    await actuators_front.ventouses_av_ext_lache()
    await actuators_front.ventouses_av_int_lache()
    await asyncio.sleep(long_timeout)
    await actuators_front.soulageur_av_down()

@robot.sequence
async def experimental_construction3_niv2():
    global poses
    ...
    build_task = asyncio.create_task(experimental_construction_niv2_au_sol_av())
    ...
    # recul
    try:
        await propulsion.reposition(-0.2, 0.4)
    except:
        await propulsion.reposition(-0.2, 0.4)
...
@robot.sequence
async def experimental_niv2_final():
    global poses
    ...
    await experimental_prise3()
    await asyncio.sleep(long_timeout)
    await experimental_goto_construction3_niv2()
    await asyncio.sleep(long_timeout)
    await experimental_construction3_niv2()
    await asyncio.sleep(long_timeout)
