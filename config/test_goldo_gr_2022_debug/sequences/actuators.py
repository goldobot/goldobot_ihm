...
@robot.sequence
async def debug_arms_enable():
    debug_goldo(__name__)

    await servos.setEnable(['epaule_g', 'coude_g', 'epaule_d', 'coude_d'], True)
    await asyncio.sleep(0.5)
    await servos.setMaxTorque(['epaule_g', 'epaule_d'], 0.00)
    await servos.setMaxTorque(['coude_g', 'coude_d'], 0.00)
    await asyncio.sleep(0.5)
