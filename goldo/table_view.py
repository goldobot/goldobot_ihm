...
class TableViewWidget(QGraphicsView):
    def __init__(self, parent = None):
        ...
        self._friend_robot = self._scene.addEllipse(0, 0, 200, 200, QPen(), QBrush(QColor('green')))
        self._friend_robot.setZValue(1)
        self._friend_robot.setPos(0 * 1000, 3 * 1000)
        self._adv1_robot = self._scene.addEllipse(0, 0, 200, 200, QPen(), QBrush(QColor('white')))
        self._adv1_robot.setZValue(1)
        self._adv1_robot.setPos(0 * 1000, 3 * 1000)
        self._adv2_robot = self._scene.addEllipse(0, 0, 200, 200, QPen(), QBrush(QColor('blue')))
    ...
    def set_client(self, client):
        ...
        self._client.adversary_telemetry.connect(self.update_adversary)

    def update_telemetry(self, telemetry):
        self._big_robot.setPos(telemetry.x * 1000, telemetry.y * 1000)
        self._big_robot.setRotation(telemetry.yaw * 180 / math.pi)

    def update_adversary(self, adversary):
        if (adversary.id == 0):
            self._friend_robot.setPos(adversary.x * 1000, adversary.y * 1000)
        elif (adversary.id == 1):
            self._adv1_robot.setPos(adversary.x * 1000, adversary.y * 1000)
        elif (adversary.id == 2):
            self._adv2_robot.setPos(adversary.x * 1000, adversary.y * 1000)
